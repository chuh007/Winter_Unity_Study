...
public class PoolManagerEditor : EditorWindow
{
    ...
    {
        string filePath = "Assets/Pool/Editor/ToolConfig.asset";
        ...
        _itemUXMLAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>($"Assets/Pool/Editor/UI/{_toolConfig.poolUxmlAssetName}");
    ...
    private void HandleAssetNameChange(PoolingItemSO item, string newName)
    {
        ...
        PoolTypeSO exist = AssetDatabase.LoadAssetAtPath<PoolTypeSO>($"{_toolConfig.poolingFolder}/{_toolConfig.typeFolder}/{newName}.asset");
        if(exist != null)
        {
    ...
    private void HandleCreateItem()
    {
        ...
        CreateIfNotExist($"{_toolConfig.poolingFolder}/{_toolConfig.typeFolder}");
        AssetDatabase.CreateAsset(typeSO, $"{_toolConfig.poolingFolder}/{_toolConfig.typeFolder}/{typeSO.typeName}.asset");

        PoolingItemSO newItem = ScriptableObject.CreateInstance<PoolingItemSO>();

        newItem.poolType = typeSO;
        
        CreateIfNotExist($"{_toolConfig.poolingFolder}/{_toolConfig.itemFolder}");
        AssetDatabase.CreateAsset(newItem, $"{_toolConfig.poolingFolder}/{_toolConfig.itemFolder}/{typeSO.typeName}.asset");
    ...
    private void GeneratePoolingItemUI()
    {
        ...
        if(_poolManager == null)
        {
            ...
            CreateIfNotExist(_toolConfig.poolingFolder);
            _poolManager.poolingItemList = new();
